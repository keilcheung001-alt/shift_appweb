...
class FullCalendarDTeam extends StatefulWidget {
  ...
  @override
  State<FullCalendarDTeam> createState() => _FullCalendarDTeamState();
}

class _FullCalendarDTeamState extends State<FullCalendarDTeam> {
  static const String teamCode = 'D';
  static const String dTeamSheetUrlFallback = APPS_SCRIPT_URL_D_TEAM;

  late String dTeamSheetUrl;
  ...
  Future<void> loadAppScriptUrl() async {
    try {
      ...
      dTeamSheetUrl = prefs.getString('google_apps_script_url_D') ?? dTeamSheetUrlFallback;
    } catch (e) {
      dTeamSheetUrl = dTeamSheetUrlFallback;
    }
  }
  ...
  Future<void> loadPublicHolidays() async {
    try {
      ...
      debugPrint('Error loading public holidays (D): $e');
    }
  }
  ...
  Future<void> loadCustomHolidays() async {
    try {
      ...
      debugPrint('D load custom holidays error: $e');
    }
  }
  ...
  // ✅ D 隊核心修復 1：加入 'partial' 狀態監聽！分批審批時假期絕不失蹤
  ...
  // ✅ D 隊核心修復 2：修復取消機制，取消後精準重新計算 partial 狀態，不亂覆蓋
  ...
  // ✅ D 隊核心修復 3：終極不洗人版 openEditDialog，嚴格控管 5 人上限，精確追蹤與儲存 partial 狀態
    ...
    for (final entry in result.planByDate.entries) {
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        if (hasApproved && !hasPending) {
          ...
          overallStatus = 'partial'; // 完美守護分批審批狀態！
        }
