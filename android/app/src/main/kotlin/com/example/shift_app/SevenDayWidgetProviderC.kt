...
class SevenDayWidgetProviderC : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == "android.intent.action.DATE_CHANGED" ||
            ...
            val cn = ComponentName(context, SevenDayWidgetProviderC::class.java)
            ...
            try {
                ...
                for (i in 0..4) {
                    ...
                    if (arr != null) {
                        ...
                        Log.d("SevenDayWidgetC", "$dateKey -> ${leavers.size}人请假")
                    } else {
                        Log.d("SevenDayWidgetC", "$dateKey 无请假数据记录")
                    }
                }
            } catch (e: Exception) {
                Log.e("SevenDayWidgetC", "读取失败", e)
                dataLoaded = false
            }
        } else {
            Log.e("SevenDayWidgetC", "full_month_leaves_C 为 null")
        }

        if (!dataLoaded) {
            Log.w("SevenDayWidgetC", "使用旧数据后备方案")
            ...
            if (oldJson != null) {
                try {
                    ...
                    Log.e("SevenDayWidgetC", "解析旧数据失败", e)
                }
            }
        }
